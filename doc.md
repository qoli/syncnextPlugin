...
> ⚠️ **注意**  
> tvOS / iOS 端的 JavaScriptCore 執行環境尚未完整實作 Promise 規範，`$http.fetch` 僅保證接受 `then(successCallback, errorCallback)` 形式的回呼。請勿依賴 `return $http.fetch(...).then().catch()` 這種 Promise 鏈式寫法，也不要把 `$http.fetch` 直接 `return` 給其他函式等待 `await`。若需要封裝請求邏輯，記得改寫成「傳入成功/失敗 callback」或在 `then` 的第一個參數內直接處理完成事件，以免在設備上遇到無法捕捉錯誤的狀況。
