...
* `host`：主域名，且仍是兼容舊版 Syncnext runtime 的核心欄位。
* `hosts`：可選的鏡像域名列表。新版 Syncnext 會在請求 URL 的 origin 命中 `host` / `hosts` 時，自動嘗試其他可用 mirror；舊版 runtime 會忽略這個欄位。
