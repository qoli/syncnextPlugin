...
> 文件責任：本文件只定義插件的技術協議與資料交接。測試層次、smoke 結果與產物管理由 [TESTING.md](./TESTING.md) 定義；新增、來源表匯出、Notion 與 Telegram 發布由 [MAINTENANCE.md](./MAINTENANCE.md) 定義；入口與最新狀態見 [README.md](./README.md)。
...
* `doc.md`：本文件，記錄協議與技術注意事項。
* `TESTING.md`：唯一的測試契約，定義驗證層次、結果判讀與 smoke 產物。
* `MAINTENANCE.md`：唯一的新增／維護／發布 runbook。
...
`node_Test.js` 是可複製的解析腳手架；`localServer.sh` 與 `test/` 用於服務 fixture；`browserify` 只在需要單檔輸出時使用。實際測試命令、何時需要 App 驗證與產物規則統一由 [TESTING.md](./TESTING.md) 定義，避免本文件複製測試流程。
...
若插件配置了 `hosts`，建議在 `app.js` 提供固定函數 `HostsProbeRequest()`，讓 runtime 在正式載入插件前完成 bootstrap probe。典型回傳結構如下：
...
* runtime 允許 redirect，驗收看的是最終 response。
* 插件正式運行時只會看到 `__syncnextPrimaryHost`。
* 不要在 JS 層自行輪詢 `hosts`，也不要假設會有 request-time host failover。
...
必須傳遞候選項目的 JSON 陣列；不可再以 `{ candidates: [...] }` 包裝。
